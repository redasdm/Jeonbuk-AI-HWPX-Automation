...
async function assertBelongsToCompany(db, table, id, companyId, label) {
    const row = await db
        .select()
        .from(table)
...
function rangeConditions(companyId, range) {
    const conditions = [eq(financeEvents.companyId, companyId)];
    if (range?.from)
        conditions.push(gte(financeEvents.occurredAt, range.from));
    if (range?.to)
        conditions.push(lte(financeEvents.occurredAt, range.to));
    return conditions;
}
export function financeService(db) {
    ...
    return {
        createEvent: async (companyId, data) => {
            ...
            if (data.costEventId)
                await assertBelongsToCompany(db, costEvents, data.costEventId, companyId, "Cost event");

...
import { plugins, pluginConfig, pluginEntities, pluginJobs, pluginJobRuns, pluginWebhookDeliveries, } from "@paperclipai/db";
...
export function pluginRegistryService(db) {
    ...
    return {
         ...
         */
        getEntityByExternalId: (pluginId, entityType, externalId) => db
            .select()
            .from(pluginEntities)
            .where(and(eq(pluginEntities.pluginId, pluginId), eq(pluginEntities.entityType, entityType), eq(pluginEntities.externalId, externalId)))
            .then((rows) => rows[0] ?? null),
        /**
        ...
        upsertEntity: async (pluginId, input) => {
            ...
            const existing = await db
                .select()
                .from(pluginEntities)
                .where(and(eq(pluginEntities.pluginId, pluginId), eq(pluginEntities.entityType, input.entityType), eq(pluginEntities.externalId, input.externalId ?? "")))
                .then((rows) => rows[0] ?? null);
            if (existing) {
                return db
         ...
         */
        createJobRun: async (pluginId, jobId, trigger) => {
            return db
         ...
         */
        createWebhookDelivery: async (pluginId, webhookKey, input) => {
            return db

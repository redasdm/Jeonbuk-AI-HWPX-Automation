...
function normalizeRelativeFilePath(candidatePath) {
...
function shouldIgnoreInstructionsEntry(entry) {
    ...
    if (!entry.isFile())
        return false;
    return (IGNORED_INSTRUCTIONS_FILE_NAMES.has(entry.name)
...
async function listFilesRecursive(rootPath) {
    ...
    async function walk(currentPath, relativeDir) {
        ...
        for (const entry of entries) {
            if (shouldIgnoreInstructionsEntry(entry))
                continue;
...
async function readFileSummary(rootPath, relativePath, entryFile) {
...
async function readLegacyInstructions(agent, config) {
...
function deriveBundleState(agent) {
    ...
    let mode = isBundleMode(storedModeRaw) ? storedModeRaw : null;
...
async function recoverManagedBundleState(agent, state) {
    ...
    if (!configuredRootMatchesManaged) {
        warnings.push(`Recovered managed instructions from disk at ${managedRootPath}; ignoring stale configured root ${state.rootPath}.`);
    }
    if (hasEntryMismatch) {
        warnings.push(`Recovered managed instructions entry file from disk as ${recoveredEntryFile}; previous entry ${state.entryFile} was missing.`);
    }
    return {
        ...state,
        mode: "managed",
...
function toBundle(agent, state, files) {
...
function applyBundleConfig(config, input) {
    const next = {
        ...config,
        [MODE_KEY]: input.mode,
...
function buildPersistedBundleConfig(derived, current, options) {
    ...
    const configMatchesRecoveredState = derived.mode === current.mode
...
async function writeBundleFiles(rootPath, files, options) {
    for (const [relativePath, content] of Object.entries(files)) {
        ...
        if (existingStat?.isFile() && !options?.overwriteExisting)
            continue;
        await fs.mkdir(path.dirname(absolutePath), { recursive: true });
        await fs.writeFile(absolutePath, content, "utf8");
    }
}
export function syncInstructionsBundleConfigFromFilePath(agent, adapterConfig) {
...
export function agentInstructionsService() {
    async function getBundle(agent) {
    ...
    async function readFile(agent, relativePath) {
        const state = await recoverManagedBundleState(agent, deriveBundleState(agent));
        ...
        if (!state.rootPath)
            throw notFound("Agent instructions bundle is not configured");
    ...
    async function ensureWritableBundle(agent, options) {
    ...
    async function updateBundle(agent, input) {
        const state = await recoverManagedBundleState(agent, deriveBundleState(agent));
    ...
    async function writeFile(agent, relativePath, content, options) {
    ...
    async function exportFiles(agent) {
        const state = await recoverManagedBundleState(agent, deriveBundleState(agent));
        if (state.rootPath) {
            const stat = await statIfExists(state.rootPath);
            if (stat?.isDirectory()) {
    ...
    async function materializeManagedBundle(agent, files, options) {

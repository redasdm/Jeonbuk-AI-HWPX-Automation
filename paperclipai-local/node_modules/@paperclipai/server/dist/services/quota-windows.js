...
function providerSlugForAdapterType(type) {
    switch (type) {
        case "claude_local":
            return "anthropic";
        case "codex_local":
...
export async function fetchAllQuotaWindows() {
    ...
    const settled = await Promise.allSettled(adapters.map((adapter) => withQuotaTimeout(adapter.type, adapter.getQuotaWindows())));
...
async function withQuotaTimeout(adapterType, task) {
    let timeoutId = null;
    try {
...
//# sourceMappingURL=quota-windows.js.map
 ...
 */
export const ORG_CHART_STYLES = ["monochrome", "nebula", "circuit", "warmth", "schematic"];
...
function guessRoleTag(node) {
    const name = node.name.toLowerCase();
    const role = node.role.toLowerCase();
    if (name === "ceo" || role.includes("chief executive"))
        return "ceo";
    if (name === "cto" || role.includes("chief technology") || role.includes("technology"))
        return "cto";
    if (name === "cmo" || role.includes("chief marketing") || role.includes("marketing"))
        return "cmo";
    if (name === "cfo" || role.includes("chief financial"))
        return "cfo";
    if (name === "coo" || role.includes("chief operating"))
        return "coo";
    if (role.includes("engineer") || role.includes("eng"))
        return "engineer";
    if (role.includes("quality") || role.includes("qa"))
        return "quality";
...
function avatarGridHeight(count) {
    if (count === 0)
        return 0;
...
function renderEmojiAvatar(cx, cy, radius, bgFill, emojiSvg, bgStroke) {
...
function defaultRenderCard(ln, theme) {
...
function renderConnectors(ln, theme) {
...
function flattenDescendants(nodes) {
    const result = [];
    for (const n of nodes) {
        result.push(n);
...
function nodesAtDepth(nodes, depth) {
    if (depth === 0)
        return nodes;
    const result = [];
    for (const n of nodes) {
 ...
 */
function estimateNextLevelWidth(parentNodes) {
    let total = 0;
 ...
 */
function collapseToAvatars(node) {
 ...
 */
function smartCollapseTree(roots) {
    // Deep clone so we can mutate
    const clone = (nodes) => nodes.map((n) => ({ ...n, reports: clone(n.reports ?? []) }));
...
export function renderOrgChartSvg(orgTree, style = "warmth", overlay) {
...
export async function renderOrgChartPng(orgTree, style = "warmth", overlay) {
...
//# sourceMappingURL=org-chart-svg.js.map